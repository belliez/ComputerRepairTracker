...
/**
 * Function to fetch organization information for the document
 */
export async function fetchOrganizationInfo(): Promise<any> {
  try {
    ...
    if (!response.ok) {
      console.error('Failed to fetch organization info for document');
      return null;
    }
